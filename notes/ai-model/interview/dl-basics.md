...
```
1. 计算输出层梯度：∂L/∂a
2. 逐层反向传递：利用链式法则 ∂L/∂W = ∂L/∂a · ∂a/∂z · ∂z/∂W
3. 累积梯度，更新参数
```
...
以 n 层网络为例，梯度传递公式：
```
∂L/∂W₁ = ∂L/∂aₙ · ∏ᵢ₌₁ⁿ (∂aᵢ/∂zᵢ · Wᵢ)
```

当 `|∂a/∂z · W| < 1` 时，连乘导致指数级衰减（消失）
当 `|∂a/∂z · W| > 1` 时，连乘导致指数级增长（爆炸）
...
```
h₁ = W₁x + b₁
h₂ = W₂h₁ + b₂ = W₂(W₁x + b₁) + b₂ = (W₂W₁)x + (W₂b₁ + b₂) = W'x + b'
```
...
| 策略 | 公式 | 特点 |
|------|------|------|
| 阶梯衰减 | ηₜ = η₀ · γ^⌊t/s⌋ | 每隔 s 步乘以 γ |
| 指数衰减 | ηₜ = η₀ · γᵗ | 连续指数衰减 |
| 余弦衰减 | ηₜ = η_min + ½(η₀-η_min)(1+cos(πt/T)) | 平滑衰减 |
...
| **Sigmoid** | 1/(1+e⁻ˣ) | 输出 (0,1)，适合概率 | 梯度消失，非零中心 |
| **Tanh** | (eˣ-e⁻ˣ)/(eˣ+e⁻ˣ) | 零中心 | 梯度消失 |
| **GELU** | x·Φ(x) | 平滑，Transformer 常用 | 计算稍复杂 |
| **Swish** | x·sigmoid(βx) | 平滑，自门控 | 需调参 |
| **SwiGLU** | Swish(xW) ⊙ (xV) | GLM/LLaMA 使用 | 参数量增加 |
...
解决方法：
- **LeakyReLU**：f(x) = max(αx, x)，α 通常为 0.01
- **PReLU**：α 作为可学习参数
...
1. **学习速度**：
   - MSE + Sigmoid：梯度 = (ŷ-y) · ŷ(1-ŷ) · x
   - CE + Sigmoid：梯度 = (ŷ-y) · x
   
   当 ŷ 接近 0 或 1 时，MSE 梯度趋近 0，学习变慢
...
# Label Smoothing
y_smooth = (1-ε)y + ε/K = [ε/4, ε/4, 1-ε+ε/4, ε/4]
```
