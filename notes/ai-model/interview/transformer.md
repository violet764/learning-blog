...
其中 n 是序列长度，d 是模型维度。n² 来自注意力矩阵的计算和存储。
...
# 加法 Attention (Bahdanau)
score(hᵢ, sⱼ) = v^T tanh(W₁hᵢ + W₂sⱼ)
```
...
|------|----------------|----------------|
| 计算复杂度 | O(nd²) 利用矩阵乘法优化 | O(n²d) 需要逐元素计算 |
...
参数量:
- W_Q: d_model × d_k × h = d_model × d_model
- W_K: d_model × d_k × h = d_model × d_model
- W_V: d_model × d_k × h = d_model × d_model
- W_O: d_model × d_model
...
```
对于频率 ω_k = 1/10000^(2k/d):

PE(pos + k) 和 PE(pos) 的关系:
sin((pos+k)ω) = sin(pos·ω)cos(k·ω) + cos(pos·ω)sin(k·ω)

这表示 PE(pos+k) 可以表示为 PE(pos) 的线性函数
模型可以通过学习这个线性变换来理解相对位置
```
...
```python
q̃_m · k̃_n = (R(m, θ)q) · (R(n, θ)k)
           = q · R(n-m, θ)k  # 只与 n-m 有关
```
...
参数量: 2 × d_model × 4d_model = 8d_model²
```
...
**以 GPT-3 175B 为例**（d_model = 12288, n_layers = 96, vocab_size = 50257）：

| 组件 | 计算公式 | 参数量 |
|------|----------|--------|
| Token Embedding | vocab × d_model | 50257 × 12288 ≈ 617M |
| Attention (每层) | 4 × d_model² | 4 × 12288² ≈ 604M |
| FFN (每层) | 2 × d_model × 4d_model | 8 × 12288² ≈ 1.2B |
| LayerNorm (每层) | 2 × 2 × d_model | 忽略不计 |
| 总参数 | embedding + layers × (attn + ffn) | ≈ 175B |

**简化估算公式**：

```
参数量 ≈ 12 × d_model² × n_layers + vocab × d_model
```
...
经验法则（LLaMA 风格）：
- 层数 n ≈ 参数量^(1/3) × 常数
- d_model ≈ 参数量^(1/3) × 另一常数

例如 7B 模型：
- LLaMA 7B: n_layers=32, d_model=4096
- 估算: 32 × 12 × 4096² ≈ 6.4B（加上词表约 7B）
...
```
KV Cache 大小 = 2 × n_layers × batch_size × seq_len × n_heads × head_dim × bytes

例如 LLaMA 7B (batch=1, seq=2048):
= 2 × 32 × 1 × 2048 × 32 × 128 × 2 (FP16)
= 1 GB
```
...
# 需要 O(n²) 的显存存储注意力矩阵
scores = Q @ K.T              # (n, n) 
attention_weights = softmax(scores)  # (n, n) 需要存储
