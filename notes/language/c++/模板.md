...
Lambda 表达式是 C++11 引入的匿名函数对象，允许在需要函数的地方直接定义函数逻辑。
...
// 值捕获 - 捕获所有外部变量的副本
auto valueCapture = [=] { return x + y; };

// 引用捕获 - 捕获所有外部变量的引用
auto refCapture = [&] { x = 100; };

// 混合捕获 - x引用捕获，y值捕获
auto mixedCapture = [&x, y] { x = 200; };

// 初始化捕获（C++14）
auto initCapture = [z = x + y] { return z; };

// mutable Lambda - 允许修改值捕获的变量副本
auto mutableLambda = [x]() mutable { x = 999; return x; };
```
...
add(5, 3);              // 整数: 8
add(2.5, 3.7);          // 浮点: 6.2
add(std::string("A"), std::string("B"));  // 字符串
```
...
std::function<void()> createCallback() {
    int localVar = 42;
    return [&localVar]() { };  // 危险！localVar已销毁
}
...
移动语义是 C++11 最重要特性之一，通过转移资源所有权避免不必要的深拷贝。
...
std::string moved = std::move(str);  // str被移动，变为空
...
// 临时对象自动移动
vec.push_back(std::string("临时"));  // 移动构造
...
2. **移动后原对象状态**：处于"有效但未定义"状态
3. **不要过度使用 move**：小对象或需要保留的值不要移动
4. **RVO 优先**：返回局部对象时，编译器会优化
