...
functools模块提供了一系列高阶函数和工具，用于操作或扩展其他函数。主要包括缓存装饰器、偏函数、函数包装工具等：

```python
# functools模块 - 高阶函数工具
# functools模块提供了一系列高阶函数和工具，用于操作或扩展其他函数
from functools import lru_cache, cache, partial, wraps, reduce, total_ordering

# lru_cache - 最近最少使用缓存装饰器
# lru_cache(maxsize) 缓存函数调用结果，maxsize为缓存大小，None表示无限制
...
print(f"fibonacci(50): {fibonacci(50)}")  # 无缓存时计算非常慢，缓存后瞬间完成

# cache - 无限缓存装饰器（Python 3.9+）
# cache() 等同于 lru_cache(maxsize=None)，适用于所有参数组合都应缓存的情况
@cache
def expensive_computation(x):
    print(f"计算 {x}...")
    return x * x

print(expensive_computation(5))  # 第一次调用，会计算
print(expensive_computation(5))  # 第二次调用，直接返回缓存结果

# partial - 偏函数
# partial(func, *args, **kwargs) 固定函数的部分参数，返回新的函数
def power(base, exp):
    return base ** exp

square = partial(power, exp=2)   # 固定exp=2，创建平方函数
cube = partial(power, exp=3)     # 固定exp=3，创建立方函数
print(f"平方: {square(5)}")      # 25
print(f"立方: {cube(3)}")        # 27

# wraps - 保留被装饰函数的元信息
# wraps(wrapped) 装饰器，用于在定义装饰器时保留原函数的__name__、__doc__等属性
def my_decorator(func):
    @wraps(func)  # 不加这行，wrapped_function的__name__会变成'wrapped_function'
...
greet("World")
print(f"函数名: {greet.__name__}")    # 输出: greet（有wraps）
print(f"文档: {greet.__doc__}")      # 输出: 问候函数（有wraps）

# reduce - 累积计算
# reduce(func, iterable, initial) 对序列元素进行累积计算
from functools import reduce
numbers = [1, 2, 3, 4, 5]
total = reduce(lambda x, y: x + y, numbers)  # 计算列表所有元素的和
print(f"求和: {total}")  # 15

# 使用initial参数
total_with_init = reduce(lambda x, y: x + y, numbers, 10)  # 从10开始累加
print(f"带初始值求和: {total_with_init}")  # 25

# total_ordering - 自动补全比较运算符
# total_ordering 类装饰器，只需定义__eq__和一个比较方法，自动生成其他比较方法
@total_ordering
class Student:
    ...
    def __lt__(self, other):  # 只定义小于，total_ordering会自动生成其他比较方法
        return self.score < other.score
...
print(f"s1 > s2: {s1 > s2}")   # False（自动生成）
```
...
| 函数名 | 功能说明 | 参数说明 | 示例 |
|--------|----------|----------|------|
| `lru_cache(maxsize)` | 最近最少使用缓存 | maxsize:缓存大小 | `@lru_cache(maxsize=128)` |
| `cache` | 无限缓存（3.9+） | 无参数 | `@cache` |
| `partial(func, *args)` | 创建偏函数 | func:原函数, args:固定参数 | `partial(int, base=2)` |
| `wraps(wrapped)` | 保留函数元信息 | wrapped:被装饰函数 | `@wraps(func)` |
| `reduce(func, iter)` | 累积计算 | func:二元函数, iter:可迭代对象 | `reduce(lambda x,y:x+y, [1,2,3])` |
| `total_ordering` | 自动补全比较运算符 | 类装饰器 | `@total_ordering` |
...
# 创建不同风格的问候函数
casual_greet = partial(greet, greeting="Hi", punctuation="~")
formal_greet = partial(greet, greeting="Good morning", punctuation=".")

print(casual_greet("Alice"))   # Hi, Alice~
print(formal_greet("Bob"))     # Good morning, Bob.
