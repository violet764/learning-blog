...
print(int('52', base=8))      # 输出42：八进制字符串转十进制
print(int('2a', base=16))     # 输出42：十六进制字符串转十进制
```
...
![IEEE 754浮点数格式](./images/Float_example.svg.png)

*图：IEEE 754浮点数格式 - 展示单精度浮点数的位布局*
...
|`round()`|按指定精度**四舍五入**，用于控制小数显示/计算精度|
...
位运算符用于对整数的二进制位进行操作，这是底层编程中常用的技术。

| 位运算符 | 作用说明 | 示例 |
|---|---|---|
| `&` | 按位与：两位都为1时结果为1，否则为 0 | `10 & 6 = 2` |
| `\|` | 按位或：任一位为1时结果为1，否则为 0 | `10 \| 6 = 14` |
| `^` | 按位异或：两位不同时结果为1，相同时为 0（同 0 异 1） | `10 ^ 6 = 12` |
| `~` | 按位取反：对所有位取反 | `~10 = -11` |
...
```python
# 集合大小 bit_count() 
print(f"集合大小：{24.bit_count()}") # 5

# 二进制长度 bit_length()
print(f"二进制长度：{24.bit_length()}") # 5
# 集合最大元素 s.bit_length()-1
print(f"集合最大元素：{24.bit_length()-1}") # 4
# 集合最小元素 (s&-s).bit_length()-1
print(f"集合最小元素：{(24&-24).bit_length()-1}") # 3
...
print("apple" in fruits)        # True o(n)判断
...
# 在集合中（查找效率高）
letters = set("python")
print("p" in letters)           # True o(1)判断
...
if age >= 18 and age <= 65 and income >= 30000:
    if has_credit_card or has_collateral:
        print("符合贷款基本条件")
    else:
        print("需要提供信用卡或抵押物")
else:
...
print(process_status(404))
print(process_status(999))
...
def greet_person(person):
    match person:
        case Person(name="Alice", age=age) if age < 30:
            return f"年轻的Alice，{age}岁"
        case Person(name=name, age=age) if age >= 65:
            return f"年长的{name}，{age}岁"
        case Person(name=name, age=age):
            return f"{name}，{age}岁"

alice = Person("Alice", 25)
bob = Person("Bob", 70)
charlie = Person("Charlie", 45)
...
print(f"range对象内存占用：{sys.getsizeof(large_range)}字节")

# range的索引和切片
numbers = range(10)
print(f"range长度：{len(numbers)}")
print(f"range[3]：{numbers[3]}")
print(f"range[2:6]：{list(numbers[2:6])}")
print(f"range[::2]：{list(numbers[::2])}")
...
`for-else` 是 Python 独有的语法结构，`else` 子句会在 `for` 循环正常结束（未被 `break` 中断）时执行；若循环被 `break` 终止，`else` 子句不会执行。若循环因 `return`、`raise` 等终止（非`break`），`else` 也不会执行
```python
for 变量 in 可迭代对象:
    循环体（可能包含 break）
else:
...
示例：
```python
# 遍历列表，未触发 break，else 执行
for i in [1, 2, 3]:
    print(i)
else:
    print("循环正常结束，未触发 break")  # 会执行

# 遍历中触发 break，else 不执行
for i in [1, 2, 3]:
    if i == 2:
        break
    print(i)
...
while total < 100:
    total += number
    number += 1
print(f"累加到{total}时停止，加了{number-1}个数")
...
while running and attempt < max_attempts:
    attempt += 1
    user_input = input("请输入密码（或'quit'退出）：")
    
    if user_input == "quit":
        running = False
        print("程序退出")
    elif user_input == "123456":
        print("密码正确！")
        running = False
    else:
...
while count < max_count:
    ...
    if count == target_count:
        print(f"达到目标计数：{target_count}")
        break
    count += 1
else:
    print("循环正常结束（未达到目标计数）")
...
# 循环与异常处理
print("\n循环与异常处理：")
files = ["file1.txt", "file2.txt", "nonexistent.txt", "file3.txt"]

for filename in files:
    try:
        with open(filename, 'r', encoding='utf-8') as file:
            content = file.read()
            print(f"成功读取{filename}，长度：{len(content)}")
    except FileNotFoundError:
...
print(f"直接遍历耗时：{end - start:.4f}秒")

# 方式3：内置函数（最快）
start = time.time()
total = sum(large_list)
end = time.time()
print(f"内置函数耗时：{end - start:.4f}秒")
