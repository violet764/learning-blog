...
|快捷键组合|功能描述|
|---|---|
|`Ctrl + B`|将选中文本设置为**粗体**（添加 `** **` 或 `__ __`）|
|`Ctrl + I`|将选中文本设置为*斜体*（添加 `* *` 或 `_ _`）|
|`Ctrl + K + V`|打开侧边预览面板，实时查看 Markdown 渲染效果|
|`Ctrl + Shift + V`|在新标签页中打开 Markdown 预览|
|`Ctrl + /`|为选中内容添加/取消行内代码（包裹 `` `  ` ``）|
|`Ctrl + Shift + K`|删除当前行|
|`Ctrl + Enter`|在当前行下方插入空行并跳转|
|`Shift + Enter`|在当前光标位置插入换行|
