...
vector<vector<int>> floyd(int n, const vector<vector<pair<int, int>>>& graph) {
    // 初始化距离矩阵：dist[i][j] 表示i到j的最短距离
...
**定义**
树状数组是一种用于高效处理**前缀和**问题的数据结构，支持单点更新和前缀查询，时间复杂度均为O(log n)。

**核心思想**
树状数组利用了**二进制的性质**，通过`lowbit`操作将数组组织成树形结构：
- `lowbit(x) = x & (-x)`：获取x的二进制表示中最低位的1所代表的值
- 例如：`lowbit(6) = lowbit(110) = 2`，`lowbit(8) = lowbit(1000) = 8`

**直观理解**
```
假设原数组 a[1..8]，树状数组 t[1..8]：
...
int main() {
    ...
    cout << "前缀和[1,4]: " << bit.query(4) << endl;  // 1+3+5+7=16
    cout << "区间和[2,5]: " << bit.rangeQuery(2, 5) << endl;  // 3+5+7+2=17
    
    bit.update(3, 2);  // 位置3加2
    cout << "更新后前缀和[1,4]: " << bit.query(4) << endl;
...
if __name__ == "__main__":
    ...
    print("前缀和[1,4]:", bit.query(4))  # 16
    print("区间和[2,5]:", bit.range_query(2, 5))  # 17
    
    bit.update(3, 2)  # 位置3加2
    print("更新后前缀和[1,4]:", bit.query(4))  # 18
```
:::
...
| 区间更新 | 需要差分技巧 | 直接支持（懒标记） |
| 适用场景 | 前缀和、逆序对 | 区间最值、区间修改 |
